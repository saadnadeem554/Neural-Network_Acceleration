...
__global__ void matrixMulKernel(float* A, float* B, float* C, float* bias,
                               int M, int N, int K) {
...
__global__ void updateParameters(float* d_W, float* d_b, float* d_W_grad, 
   float* d_b_grad, int rows, int cols, float learning_rate) 
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
float* d_output, float* d_target) 
{
    ...
    computeGradients<<<numBlocks, blockSize>>>(d_output, d_target, d_hidden, d_input,d_W2_grad, d_b2_grad, d_W1_grad, d_b1_grad,net->d_W2, net->d_W1,HIDDEN_SIZE, INPUT_SIZE, OUTPUT_SIZE);

    // Update parameters of the network
    updateParameters<<<(HIDDEN_SIZE * INPUT_SIZE + blockSize - 1) / blockSize, blockSize>>>(net->d_W1, net->d_b1, d_W1_grad, d_b1_grad,HIDDEN_SIZE, INPUT_SIZE, LEARNING_RATE);

    updateParameters<<<(OUTPUT_SIZE * HIDDEN_SIZE + blockSize - 1) / blockSize, blockSize>>>(net->d_W2, net->d_b2, d_W2_grad, d_b2_grad,OUTPUT_SIZE, HIDDEN_SIZE, LEARNING_RATE);
...
void forward(NeuralNetwork* net, float* d_input, float* d_hidden, float* d_output) {
    dim3 blockDim(16, 16);
    dim3 gridDim((INPUT_SIZE + blockDim.x - 1) / blockDim.x,
                 (HIDDEN_SIZE + blockDim.y - 1) / blockDim.y);

    // Hidden layer
    matrixMulKernel<<<gridDim, blockDim>>>(net->d_W1, d_input, d_hidden, 
                                          net->d_b1, HIDDEN_SIZE, INPUT_SIZE, 1);
    reluKernel<<<(HIDDEN_SIZE + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE>>>(
        d_hidden, HIDDEN_SIZE);

    // Output layer
    gridDim.x = (HIDDEN_SIZE + blockDim.x - 1) / blockDim.x;
    gridDim.y = (OUTPUT_SIZE + blockDim.y - 1) / blockDim.y;
    matrixMulKernel<<<gridDim, blockDim>>>(net->d_W2, d_hidden, d_output,
                                          net->d_b2, OUTPUT_SIZE, HIDDEN_SIZE, 1);
    softmaxKernel<<<1, BLOCK_SIZE>>>(d_output, OUTPUT_SIZE);
}
//  the train function to include loss calculation
