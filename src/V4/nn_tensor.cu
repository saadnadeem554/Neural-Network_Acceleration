...
__global__ void batchFCKernelWMMA(half* weights, half* inputs, float* outputs, half* bias,
    int output_size, int input_size, int batch_size) {
    // Calculate grid position
    int warpM = (blockIdx.y * blockDim.y + threadIdx.y) / 32;
    int warpN = blockIdx.z; // Use the batch dimension directly
    ...
    // Check bounds
    if (warpM * WMMA_M >= output_size || warpN >= batch_size) return;
    ...
    // Loop over input dimension in tiles of WMMA_K
    for (int k = 0; k < input_size; k += WMMA_K) {
        if (k + WMMA_K <= input_size) {
            // Get pointers to current tiles
            const half* a_tile = weights + warpM * WMMA_M * input_size + k;
            const half* b_tile = inputs + warpN * input_size + k;
            
            // Load matrix tiles
            wmma::load_matrix_sync(a_frag, a_tile, input_size);
            wmma::load_matrix_sync(b_frag, b_tile, 1);
    ...
    float* c_output = outputs + warpN * output_size + warpM * WMMA_M;
    wmma::store_matrix_sync(c_output, c_frag, output_size, wmma::mem_row_major);
    
    // Add bias and apply ReLU
    int lane_id = threadIdx.x % 32;
    int row = warpM * WMMA_M + lane_id;
    if (lane_id < WMMA_M && row < output_size) {
        c_output[lane_id] += __half2float(bias[row]);
    }
}
...
__global__ void floatToHalfKernel(const float* input, __half* output, int size) {
...
void convertFloatToHalf(const float* d_input_float, __half* d_output_half, int size, cudaStream_t stream = 0) {
    int threads = 256;
    int blocks = (size + threads - 1) / threads;
    floatToHalfKernel<<<blocks, threads, 0, stream>>>(d_input_float, d_output_half, size);
}

// Allocate these once and reuse
static __half *d_W2_half = NULL, *d_b2_half = NULL;
static __half *d_batch_hidden_half = NULL;
...
// Initialize half-precision buffers if not already done
if (d_W2_half == NULL) {
cudaMalloc(&d_W2_half, OUTPUT_SIZE * HIDDEN_SIZE * sizeof(__half));
cudaMalloc(&d_b2_half, OUTPUT_SIZE * sizeof(__half));
cudaMalloc(&d_batch_hidden_half, BATCH_SIZE * HIDDEN_SIZE * sizeof(__half));

// Initial conversion of weights
convertFloatToHalf(net->d_W2, d_W2_half, OUTPUT_SIZE * HIDDEN_SIZE);
convertFloatToHalf(net->d_b2, d_b2_half, OUTPUT_SIZE);
}

// First layer remains in full precision
dim3 blockDim(1, 256);
...
convertFloatToHalf(d_batch_hidden, d_batch_hidden_half, HIDDEN_SIZE * batchSize, stream);

// Second layer using Tensor Cores
dim3 gridDim2((batchSize + 15) / 16, (OUTPUT_SIZE + 15) / 16, 1);
dim3 blockDim2(32, 8); // 32 threads per warp, 8 warps per block

batchFCKernelWMMA<<<gridDim2, blockDim2, 0, stream>>>(
d_W2_half, d_batch_hidden_half, d_batch_output, d_b2_half,
